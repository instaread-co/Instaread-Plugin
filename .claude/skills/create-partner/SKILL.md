...
- **`min_php_version`** — string like `"5.6"`, `"7.4"`, `"8.1"`. **Informational only** — records the partner's PHP runtime as of onboarding. **Not enforced anywhere in core.** Purpose: when a partner reports issues, this tells us at a glance whether they're on an old runtime that may lack PHP 7+ features our code assumes (48 `??` uses require PHP 7.0+). If a partner is stuck on PHP < 7.0, the **preferred** answer is still "please upgrade PHP" — 5.6 has been EOL since Jan 2019 and is a security exposure. If they truly can't, we can hand them a **one-off legacy zip** built via `tools/legacy-php-transpile/` (docker + `php:5.6-cli` verified). Never merge the transpiled file back into `core/` — main-branch core stays PHP 7+. First partner to carry this field: agdaily (PHP 5.6.40 on 2026-08-25 during onboarding; legacy zip shipped as a one-off, auto-updates disabled).
...
| Slot+player tag IN DOM, but iframe doesn't render. `instaread.<pub>.js` missing from network. Combined file `hummingbird-assets/<hash>.js` is in page. | **Hummingbird** (WPMU Dev). Same root cause family as SG Optimizer. Pre-v4.7.11 we only excluded `wphb_minify_resource`, leaving `wphb_combine_resource` wide open. Our bundle was inlined into the combined file. Observed on asamnews.com 2026-06-30 (Hummingbird-combined file at 31KB contained full `InstareadPlayer` class at offset ~9k). | Ensure partner is on v4.7.11+. v4.7.11 hooks all 5 per-resource filters (combine/minify/defer/async/block) + `wphb_dont_combine_handles`. After upgrade, purge Hummingbird cache via wp-admin → Hummingbird → Asset Optimization → "Clear Asset Cache". Verify the new `hummingbird-assets/<hash>.js` does NOT contain `class InstareadPlayer`. |
